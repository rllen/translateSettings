...
public class Log {
    ...
    private static final String LOG_DIR = ".translator/logs/";
    ...
    private static final File sDir = new File(Environment.getExternalStorageDirectory(), LOG_DIR);
    ...
    private static File getLogFile() {
        File file = new File(Environment.getExternalStorageDirectory(), String.format(LOG_DIR + "Log_%s_%s.log", sFormat1.format(new Date()), Process.myPid()));
    ...
    static {
        sHandlerThread = new HandlerThread("translate@FileLogThread");
    ...
    private static void println(final int level, final String tag, final String format, final Object[] args, final Throwable tr) {
        ...
        if (tr != null) {
            message += android.util.Log.getStackTraceString(tr); // Get trace.
        }
