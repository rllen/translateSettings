...
class TimePickerSpinnerDelegate extends TimePicker.AbstractTimePickerDelegate {
    ...
    private static final String LOCALE_DATA_CLASS_NAME = "libcore.icu.LocaleData";
    // state
    ...
    public TimePickerSpinnerDelegate(TimePicker delegator, Context context, AttributeSet attrs,
        ...
        try {
            NumberPicker.Formatter formatter = (NumberPicker.Formatter) MethodUtils.invokeStaticMethod(NumberPicker.class, "getTwoDigitFormatter");
...
//        }
    ...
    public static String[] getAmPmStrings(Context context) {
        try {
            ...
            Class cls = Class.forName(LOCALE_DATA_CLASS_NAME);
            Object d = MethodUtils.invokeStaticMethod(cls, "get", new Object[]{context.getResources().getConfiguration().locale});
            Field amPmField = cls.getField("amPm");
            if (!amPmField.isAccessible()) {
                amPmField.setAccessible(true);
            }
            Field narrowAmField = cls.getField("narrowAm");
            if (!narrowAmField.isAccessible()) {
                narrowAmField.setAccessible(true);
            }
            Field narrowPmField = cls.getField("narrowPm");
            if (!narrowPmField.isAccessible()) {
                narrowPmField.setAccessible(true);
            }
            String[] amPm = (String[]) amPmField.get(d);
            String narrowAm = (String) narrowAmField.get(d);
            String narrowPm = (String) narrowPmField.get(d);
            result[0] = amPm[0].length() > 4 ? narrowAm : amPm[0];
            result[1] = amPm[1].length() > 4 ? narrowPm : amPm[1];
            return result;
        } catch (Exception e) {
            e.printStackTrace();
        }
